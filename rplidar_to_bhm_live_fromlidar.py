...
def read_lidar(num_of_scans_to_stack, itera=0):
    ...
    path = '/home/pi/Desktop/SURF/datasets/lidar_store.npy'
    ...
    rp = lidar
    #np.delete(np.array(lidar_data), [0,1000], axis=0)
    #np.delete(np.array(lidar_data), [0,1000], axis=1)
    for i, scan in enumerate(lidar.iter_scans()):
        ...
        if i > 4:
            ...
            lidar.disconnect()
            break
    np.save(path, np.array(lidar_data))
            
    # Read data from rplidar file
    #filename = 'examples'
    #print(" |_reading LIDAR from " + 'datasets/' + filename + '.npy')
    rp_lidar_data = np.load(path, allow_pickle=True)
