...
def webcam_inf(interpreter, anchors, classes, threshold=0.25):
    ...
    out = cv2.VideoWriter('real.avi', fourcc, 20.0, (640,480))
    count = 0
    while True:
        count +=1
        # Read frame from webcam
        ret, frame = cap.read() 
        #cv2.imwrite('photos.jpg',frame)
        #cv2.imshow()
        out.write(frame)
...
def video_inf(interpreter, anchors, classes, path, threshold=0.25):
    ...
    #cap = cv2.ROTATE_90_CLOCKWISE
    
    #cap=cv2.Transpose(cap,timg)
    #cap=cv2.Flip(timg,timg,flipMode=1)
    ...
    ret, frame = cap.read()
    fshape = frame.shape
    fheight = fshape[0] #if flipped 90 degrees, make 1
    fwidth = fshape[1] # if flipped 90 degrees make 0
    print(fwidth , fheight)
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter('output.avi',fourcc, 25, (fwidth,fheight))
    #fourcc = cv2.VideoWriter_fourcc(*'XVID') was 12.0
    #out = cv2.VideoWriter('real.avi', fourcc, 20.0, (640,480))
    count = 0
    while True:
        count+=1
        ...
        ret, frame = cap.read()
        ##frame = cv2.rotate(frame, cv2.cv2.ROTATE_90_CLOCKWISE) #rotate frame WORKS, UNCOMMENT
        #cv::rotate(frame, frame, cv::ROTATE_90_CLOCKWISE);
        #img_rotate_90_clockwise = cv2.rotate(cap.read(), cv2.ROTATE_90_CLOCKWISE)
        #cv2.imwrite(cap.read(), img_rotate_90_clockwise)
        # True
        ...
        #frame = cv2.rotate(cap, cv2.cv2.ROTATE_90_CLOCKWISE)
